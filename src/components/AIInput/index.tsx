...
import { Network, ChevronRight, Send, SquareM } from 'lucide-react';
...
export default function Mention({
    ...
    onChange,
    onSend,
    placeholder = '# 可以关联', children }: MentionProps) {
    ...
    const onSelectedPath = (path: string[]) => {
        ...
        let tabs = path.map((tab, index) => {
            ...
            return (`<span class="ai-tag" title="${normalizePath(text)}" contenteditable="false">${normalizePath(text)}</span >`)
        })
    ...
    return (
        <div id='MentionBox' className='relative'>
            ...
            {/* 输入编辑器 styles*/}
            <div
                className={` Mention w-full min-h-36 max-h-36 scrollbar-hide overflow-y-auto rounded-lg p-3 text-base border border-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-400`}
                ref={editorRef}
                contentEditable
                onInput={handleInput}
            ...
            <div className='absolute bottom-2 right-2 z-10 space-x-2'>
                ...
                <Button className='cursor-pointer' onClick={handleSend} size='xs'>
